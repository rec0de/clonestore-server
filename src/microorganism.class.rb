...
class Microorganism attr_reader :id, :createdBy, :initials, :labNotes, :organism, :plasmid, :resistance, :timeOfEntry, :timeOfCreation

	def initialize(createdBy, initials, labnotes, organism, plasmid, resistance, timeCreated, timeOfEntry = nil, id = nil)
		@createdBy = createdBy
		@initials = initials
		@labNotes = labnotes
		@organism = organism
	...
	def sanityCheck
		...
		if @createdBy == nil || @initials == nil
			raise CloneStorePlasmidSanityError, 'Creator Name and Initials of microorganism have to be set'
		end
	...
	def self.fromHash(h)
		Microorganism.new(h['createdBy'], h['initials'], h['labNotes'], h['organism'], h['plasmid'], h['resistance'], h['timeOfCreation'], h['timeOfEntry'], h['id'])
	end
	...
	def self.fromJSON(json)
		begin
			...
			raise CloneStorePlasmidSanityError, "Microorganism JSON data is corrupt"
		end
